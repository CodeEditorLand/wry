...
fn main() {
	let wry_data =
		&utils::wry_root_path().join("gh-pages").join("wry-data.json");
	let wry_recent =
		&utils::wry_root_path().join("gh-pages").join("wry-recent.json");
	...
	let current_data:utils::BenchResult =
		serde_json::from_reader(current_data_buffer)
			.expect("Unable to read current data buffer");

	// all data's
	let all_data_buffer = BufReader::new(
		File::open(wry_data).expect("Unable to read all data file"),
	);
	...
	utils::write_json(
		...
		&serde_json::to_value(&all_data)
			.expect("Unable to build final json (all)"),
	)
	.unwrap_or_else(|_| panic!("Unable to write {:?}", wry_data));

	utils::write_json(
		wry_recent.to_str().expect("Something wrong with wry_recent"),
