	...
	let (device, queue) = adapter
		.request_device(
			&wgpu::DeviceDescriptor {
				...
				// Make sure we use the texture resolution limits from the
				// adapter, so we can support images the size of the swapchain.
		...
		.run(|event, evl| {
			...
			match event {
				Event::WindowEvent {
					...
					surface.configure(&device, &config);
					// On macos the window needs to be redrawn manually after
					// resizing
