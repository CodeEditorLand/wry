...
//!
//! The webview requires a running event loop and a window type that implements
//! [`HasWindowHandle`], or a gtk container widget if you need to support X11
//! and Wayland. You can use a windowing library like [`tao`] or [`winit`].
//!
//! ## Examples
//!
//! This example leverages the [`HasWindowHandle`] and supports Windows, macOS,
//! iOS, Android and Linux (X11 Only). See the following example using
//! [`winit`].
//!
//! ```no_run
...
//!
//! let webview = WebViewBuilder::new().with_url("https://tauri.app").build(&window).unwrap();
//! ```
//!
//! If you also want to support Wayland too, then we recommend you use
//! [`WebViewBuilderExtUnix::new_gtk`] on Linux. See the following example using
//! [`tao`].
//!
//! ```no_run
...
//! ## Child webviews
//!
//! You can use [`WebView::new_as_child`] or [`WebViewBuilder::new_as_child`] to
//! create the webview as a child inside another window. This is supported on
...
//! let window = WindowBuilder::new().build(&event_loop).unwrap();
//!
//! let builder = WebViewBuilder::new().with_url("https://tauri.app").with_bounds(Rect {
//! 	position:LogicalPosition::new(100, 100).into(),
//! 	size:LogicalSize::new(200, 200).into(),
//! });
//!
...
//!
//! Note that on Linux, we use webkit2gtk webviews so if the windowing library
//! doesn't support gtk (as in [`winit`]) you'll need to call [`gtk::init`]
//! before creating the webview and then call [`gtk::main_iteration_do`]
//! alongside your windowing library event loop.
//!
//! ```no_run
...
//! ## Android
//!
//! In order for `wry` to be able to create webviews on Android, there is a few
//! requirements that your application needs to uphold:
//! 1. You need to set a few environment variables that will be used to generate
//!    the necessary kotlin
//! files that you need to include in your Android application for wry to
//! function properly.
//!     - `WRY_ANDROID_PACKAGE`: which is the reversed domain name of your
//!       android project and the app name in snake_case, for example,
//!       `com.wry.example.wry_app`
//!     - `WRY_ANDROID_LIBRARY`: for example, if your cargo project has a lib
//!       name `wry_app`, it will generate `libwry_app.so` so you se this env
//!       var to `wry_app`
...
//!    preferably it should use the generated `WryActivity` or inherit it.
...
//! 4. Your Rust app needs to call `wry::android_binding!` macro to setup the
//!    JNI functions that will be called by `WryActivity` and various other
//!    places.
//!
...
//!
//! - `os-webview` (default): Enables the default WebView framework on the
//!   platform. This must be enabled
//! for the crate to work. This feature was added in preparation of other ports
//! like cef and servo.
//! - `protocol` (default): Enables [`WebViewBuilder::with_custom_protocol`] to
//!   define custom URL scheme for handling tasks like
//! loading assets.
//! - `drag-drop` (default): Enables [`WebViewBuilder::with_drag_drop_handler`]
//!   to control the behaviour when there are files
//! interacting with the window.
//! - `devtools`: Enables devtools on release builds. Devtools are always
//!   enabled in debug builds.
//! On **macOS**, enabling devtools, requires calling private apis so you should
//! not enable this flag in release build if your app needs to publish to App
//! Store.
//! - `transparent`: Transparent background on **macOS** requires calling
//!   private functions.
//! Avoid this in release build if your app needs to publish to App Store.
//! - `fullscreen`: Fullscreen video and other media on **macOS** requires
...
//! - `tracing`: enables [`tracing`] for `evaluate_script`, `ipc_handler` and
//!   `custom_protocols.
//!
...
///
/// See [`WebViewBuilder::with_asynchronous_custom_protocol`] for more
/// information.
pub struct RequestAsyncResponder {
	pub(crate) responder:Box<dyn FnOnce(Response<Cow<'static, [u8]>>)>,
}

// SAFETY: even though the webview bindings do not indicate the responder is
// Send, it actually is and we need it in order to let the user do the protocol
// computation on a separate thread or async task.
...
pub struct WebViewAttributes<'a> {
	/// An id that will be passed when this webview makes requests in certain
	/// callbacks.
	...
	/// Specify the webview background color. This will be ignored if
	...
	/// ## PLatform-specific:
	///
	/// - **Windows:** the string can not be larger than 2 MB (2 * 1024 * 1024
	///   bytes) in total size
	pub html:Option<String>,

	/// A list of initialization javascript scripts to run when loading new
	/// pages. When webview load a new page, this initialization code will be
	/// executed. It is guaranteed that code is executed before
	/// `window.onload`.
	///
	/// Second parameter represents if script should be added to main frame only
	/// or sub frames also. `true` for main frame only, `false` for sub frames.
	///
	/// ## Platform-specific
	///
	/// - **Android:** The Android WebView does not provide an API for
	///   initialization scripts,
	/// so we prepend them to each HTML head. They are only implemented on
	/// custom protocol URLs.
	pub initialization_scripts:Vec<(String, bool)>,

	/// A list of custom loading protocols with pairs of scheme uri string and a
	/// handling closure.
	///
	/// The closure takes an Id ([WebViewId]), [Request] and
	/// [RequestAsyncResponder] as arguments and returns a [Response].
	///
	/// # Note
	///
	/// If using a shared [WebContext], make sure custom protocols were not
	/// already registered on that web context on Linux.
	///
	/// # Warning
	///
	...
	/// exact same Origin header in `Access-Control-Allow-Origin` if you wish
	/// to send requests with native `fetch` and `XmlHttpRequest` APIs. Here are
	/// the different Origin headers across platforms:
	///
	/// - macOS, iOS and Linux: `<scheme_name>://<path>` (so it will be
	///   `wry://path/to/page/`).
	/// - Windows and Android: `http://<scheme_name>.<path>` by default (so it will
	///   be `http://wry.path/to/page). To use `https` instead of `http`, use
	...
	/// - iOS: To get the path of your assets, you can call [`CFBundle::resources_path`](https://docs.rs/core-foundation/latest/core_foundation/bundle/struct.CFBundle.html#method.resources_path).
	///   So url like `wry://assets/index.html` could get the html file in
	///   assets directory.
	...
	///
	/// The closure take a `String` parameter as url and returns a `bool` to
	/// determine whether the navigation should happen. `true` allows to
	/// navigate and `false` does not.
	pub navigation_handler:Option<Box<dyn Fn(String) -> bool>>,

	/// A download started handler to manage incoming downloads.
	///
	/// The closure takes two parameters, the first is a `String` representing
	/// the url being downloaded from and and the second is a mutable `PathBuf`
	/// reference that (possibly) represents where the file will be downloaded
	/// to. The latter parameter can be used to set the download location by
	/// assigning a new path to it, the assigned path _must_ be absolute. The
	/// closure returns a `bool` to allow or deny the download.
	...
	///
	/// The closure is fired when the download completes, whether it was
	/// successful or not. The closure takes a `String` representing the URL of
	/// the original download request, an `Option<PathBuf>` potentially
	/// representing the filesystem path the file was downloaded to, and a
	/// `bool` indicating if the download succeeded. A value of `None` being
	/// passed instead of a `PathBuf` does not necessarily indicate that the
	/// download did not succeed, and may instead indicate some other failure,
	/// always check the third parameter if you need to know if the download
	/// succeeded.
	///
	/// ## Platform-specific:
	///
	/// - **macOS**: The second parameter indicating the path the file was saved
	///   to, is always empty,
	/// due to API limitations.
	pub download_completed_handler:Option<Rc<dyn Fn(String, Option<PathBuf>, bool) + 'static>>,

	/// A new window handler to decide if incoming url is allowed to open in a
	/// new window.
	///
	/// The closure take a `String` parameter as url and return `bool` to
	/// determine whether the window should open. `true` allows to open and
	/// `false` does not.
	...
	/// your app will still need to add menu item accelerators to use the
	/// clipboard shortcuts.
	pub clipboard:bool,
	...
	/// ## Platform-specific
	///
	/// - macOS: This will call private functions on **macOS**. It is enabled in
	///   **debug** builds,
	/// but requires `devtools` feature flag to actually enable it in
	/// **release** builds.
	/// - Android: Open `chrome://inspect/#devices` in Chrome to get the
	///   devtools window. Wry's `WebView` devtools API isn't supported on
	///   Android.
	/// - iOS: Open Safari > Develop > [Your Device Name] > [Your WebView] to
	...
	/// Indicates whether horizontal swipe gestures trigger backward and forward
	/// page navigation.
	///
	/// ## Platform-specific:
	///
	/// - Windows: Setting to `false` does nothing on WebView2 Runtime version
	///   before 92.0.902.0,
	...
	/// Set a handler closure to process the change of the webview's document
	/// title.
	pub document_title_changed_handler:Option<Box<dyn Fn(String)>>,

	/// Run the WebView with incognito mode. Note that WebContext will be
	/// ingored if incognito is enabled.
	///
	/// ## Platform-specific:
	///
	/// - Windows: Requires WebView2 Runtime version 101.0.1210.39 or higher,
	...
	/// Set a proxy configuration for the webview. Supports HTTP CONNECT and
	/// SOCKSv5 proxies
	///
	/// - **macOS**: Requires macOS 14.0+ and the `mac-proxy` feature flag to be
	///   enabled.
	...
	/// This is only effective if the webview was created by
	/// [`WebView::new_as_child`] or [`WebViewBuilder::new_as_child`]
	/// or on Linux, if was created by [`WebViewExtUnix::new_gtk`] or
	/// [`WebViewBuilderExtUnix::new_gtk`] with [`gtk::Fixed`].
...
///
/// [`WebViewBuilder`] / [`WebView`] are the basic building blocks to construct
/// WebView contents and scripts for those who prefer to control fine grained
/// window creation and event handling. [`WebViewBuilder`] provides ability to
...
impl<'a> WebViewBuilder<'a> {
	...
	/// Create a new [`WebViewBuilder`] with a web context that can be shared
	/// with multiple [`WebView`]s.
	...
	where
		...
		Self { inner:self.inner.and_then(func) }
	}

	/// Set an id that will be passed when this webview makes requests in
	/// certain callbacks.
	...
	/// we prepend them to each HTML head (implementation only supported on
	/// custom protocol URLs). For remote URLs, we use [onPageStarted] which is
	/// not guaranteed to run before other scripts.
	///
	...
	/// Same as [`with_initialization_script`](Self::with_initialization_script)
	/// but with option to inject into main frame only or sub frames.
	///
	/// ## Example
	/// ```no_run
	/// # use wry::{WebViewBuilder, raw_window_handle, Rect, dpi::*};
	...
	/// 		"console.log('Running inside main frame only')",
	/// 		true,
	/// 	)
	/// 	.with_initialization_script_for_main_only(
	/// 		"console.log('Running  main frame and sub frames')",
	/// 		false,
	/// 	)
	/// 	.with_url("https://tauri.app")
	/// 	.build(&window)
	...
	pub fn with_initialization_script_for_main_only(self, js:&str, main_only:bool) -> Self {
		self.and_then(|mut b| {
			if !js.is_empty() {
				b.attrs.initialization_scripts.push((js.to_string(), main_only));
			}
			Ok(b)
		})
	}

	/// Register custom loading protocols with pairs of scheme uri string and a
	/// handling closure.
	///
	/// The closure takes a [Request] and returns a [Response]
	///
	/// When registering a custom protocol with the same name, only the last
	/// regisered one will be used.
	///
	/// # Warning
	///
	/// Pages loaded from custom protocol will have different Origin on
	...
	/// # Reading assets on mobile
	///
	/// - Android: For loading content from the `assets` folder (which is copied
	///   to the Andorid apk) please
	/// use the function [`with_asset_loader`] from [`WebViewBuilderExtAndroid`]
	/// instead. This function on Android can only be used to serve assets you
	/// can embed in the binary or are elsewhere in Android (provided the app
	/// has appropriate access), but not from the `assets` folder which lives
	/// within the apk. For the cases where this can be used, it works the same
	/// as in macOS and Linux.
	/// - iOS: To get the path of your assets, you can call [`CFBundle::resources_path`](https://docs.rs/core-foundation/latest/core_foundation/bundle/struct.CFBundle.html#method.resources_path).
	///   So url like `wry://assets/index.html` could get the html file in
	///   assets directory.
	...
	/// Same as [`Self::with_custom_protocol`] but with an asynchronous
	/// responder.
	///
	/// When registering a custom protocol with the same name, only the last
	/// regisered one will be used.
	...
	/// ```no_run
	/// use wry::{WebViewBuilder, raw_window_handle};
	/// WebViewBuilder::new().with_asynchronous_custom_protocol(
	/// 	"wry".into(),
	...
	/// ## Platform-specific
	///
	/// - **Linux / Android**: The request URL is not supported on iframes and
	///   the main frame URL is used instead.
	...
	/// Set a handler closure to process incoming [`DragDropEvent`] of the
	/// webview.
	///
	/// # Blocking OS Default Behavior
	/// Return `true` in the callback to block the OS' default behavior.
	...
	/// Load the provided URL with given headers when the builder calling
	/// [`WebViewBuilder::build`] to create the [`WebView`]. The provided URL
	/// must be valid.
	///
	/// ## Note
	///
	/// Data URLs are not supported, use [`html`](Self::with_html) option
	/// instead.
	...
	/// to create the [`WebView`]. The provided URL must be valid.
	///
	/// ## Note
	///
	/// Data URLs are not supported, use [`html`](Self::with_html) option
	...
	/// ## Platform-specific
	///
	/// - Windows: Requires WebView2 Runtime version 86.0.616.0 or higher, does
	...
	/// ## Platform-specific
	///
	/// - Windows: Setting to `false` can't disable pinch zoom on WebView2
	///   Runtime version before 91.0.865.0,
	...
	/// Set a navigation handler to decide if incoming url is allowed to
	/// navigate.
	///
	/// The closure take a `String` parameter as url and returns a `bool` to
	/// determine whether the navigation should happen. `true` allows to
	...
	//// The closure takes two parameters, the first is a `String` representing the url being
	//// downloaded from and and the
	/// second is a mutable `PathBuf` reference that (possibly) represents where
	/// the file will be downloaded to. The latter parameter can be used to set
	/// the download location by assigning a new path to it, the assigned path
	/// _must_ be absolute. The closure returns a `bool` to allow or deny the
	/// download.
	...
	/// Sets a download completion handler to manage downloads that have
	/// finished.
	///
	/// The closure is fired when the download completes, whether it was
	/// successful or not. The closure takes a `String` representing the URL of
	...
	/// Set a new window request handler to decide if incoming url is allowed to
	/// be opened.
	///
	/// The closure take a `String` parameter as url and return `bool` to
	/// determine whether the window should open. `true` allows to open and
	...
	///
	/// - **macOS**: Requires macOS 14.0+ and the `mac-proxy` feature flag to be
	///   enabled. Supports HTTP CONNECT and SOCKSv5 proxies.
	...
	/// Specify the webview position relative to its parent if it will be
	/// created as a child or if created using
	/// [`WebViewBuilderExtUnix::new_gtk`] with [`gtk::Fixed`].
	///
	...
	/// Consume the builder and create the [`WebView`] from a type that
	/// implements [`HasWindowHandle`].
	///
	/// # Platform-specific:
	///
	/// - **Linux**: Only X11 is supported, if you want to support Wayland too,
	///   use [`WebViewBuilderExtUnix::new_gtk`].
	///
	///   Although this methods only needs an X11 window handle, we use
	/// webkit2gtk, so you still need to initialize gtk   by callling
	/// [`gtk::init`] and advance its loop alongside your event loop using
	/// [`gtk::main_iteration_do`].   Checkout the [Platform Considerations](https://docs.rs/wry/latest/wry/#platform-considerations) section in the crate root documentation.
	/// - **Windows**: The webview will auto-resize when the passed handle is
	///   resized.
	/// - **Linux (X11)**: Unlike macOS and Windows, the webview will not
	///   auto-resize and you'll need to call [`WebView::set_bounds`] manually.
	///
	/// # Panics:
	///
	...
	/// Consume the builder and create the [`WebView`] as a child window inside
	/// the provided [`HasWindowHandle`].
	///
	/// ## Platform-specific
	///
	/// - **Windows**: This will create the webview as a child window of the
	///   `parent` window.
	/// - **macOS**: This will create the webview as a `NSView` subview of the
	///   `parent` window's
	/// content view.
	/// - **Linux**: This will create the webview as a child window of the
	///   `parent` window. Only X11
	/// is supported. This method won't work on Wayland.
	///
	///   Although this methods only needs an X11 window handle, you use
	/// webkit2gtk, so you still need to initialize gtk   by callling
	/// [`gtk::init`] and advance its loop alongside your event loop using
	/// [`gtk::main_iteration_do`].   Checkout the [Platform Considerations](https://docs.rs/wry/latest/wry/#platform-considerations) section in the crate root documentation.
	///
	///   If you want to support child webviews on X11 and Wayland at the same
	/// time,   we recommend using [`WebViewBuilderExtUnix::new_gtk`] with
	/// [`gtk::Fixed`].
	/// - **Android/iOS:** Unsupported.
	///
	/// # Panics:
...
pub trait WebViewBuilderExtDarwin {
	...
	/// Can be used as a replacement for data_directory not being available in
	/// WKWebView.
	///
...
pub trait WebViewBuilderExtWindows {
	...
	/// ## Warning
	///
	/// - Webview instances with different browser arguments must also have
	///   different [data directories](struct.WebContext.html#method.new).
	/// - By default wry passes
	...
	/// so if you use this method, you have to add these arguments yourself if
	/// you want to keep the same behavior.
	fn with_additional_browser_args<S:Into<String>>(self, additional_args:S) -> Self;

	/// Determines whether browser-specific accelerator keys are enabled. When
	/// this setting is set to `false`, it disables all accelerator keys that
	/// access features specific to a web browser. The default value is `true`.
	/// See the following link to know more details.
	///
	/// Setting to `false` does nothing on WebView2 Runtime version before
	/// 92.0.902.0, see https://learn.microsoft.com/en-us/microsoft-edge/webview2/release-notes/archive?tabs=dotnetcsharp#10824-prerelease
	///
	...
	/// Specifies the theme of webview2. This affects things like
	/// `prefers-color-scheme`.
	///
	/// Defaults to [`Theme::Auto`] which will follow the OS defaults.
	///
	/// Requires WebView2 Runtime version 101.0.1210.39 or higher, does nothing
	/// on older versions, see https://learn.microsoft.com/en-us/microsoft-edge/webview2/release-notes/archive?tabs=dotnetcsharp#10121039
	...
	///
	/// Using a `http` scheme will allow mixed content when trying to fetch
	/// `http` endpoints and is therefore less secure but will match the
	/// behavior of the `<scheme>://path/to/page` protocols used on macOS and
	/// Linux.
	///
	...
	/// CSS styles that modify the scrollbar are applied on top of the native
	/// appearance configured here.
	///
	...
	/// on older versions, see https://learn.microsoft.com/en-us/microsoft-edge/webview2/release-notes/?tabs=dotnetcsharp#10253541
	fn with_scroll_bar_style(self, style:ScrollBarStyle) -> Self;

	/// Determines whether the ability to install and enable extensions is
	/// enabled.
	///
	/// By default, extensions are disabled.
	///
	/// Requires WebView2 Runtime version 1.0.2210.55 or higher, does nothing on
	/// older versions, see https://learn.microsoft.com/en-us/microsoft-edge/webview2/release-notes/archive?tabs=dotnetcsharp#10221055
	fn with_browser_extensions_enabled(self, enabled:bool) -> Self;

	/// Set the path from which to load extensions from. Extensions stored in
	/// this path should be unpacked.
	///
	/// Does nothing if browser extensions are disabled. See
	/// [`with_browser_extensions_enabled`](Self::with_browser_extensions_enabled)
...
pub trait WebViewBuilderExtAndroid {
	...
	/// to load assets from Android's `asset` folder when using `with_url` as
	/// `<protocol>://assets/` (e.g.: `wry://assets/index.html`). Note that
	/// this registers a custom protocol with the provided String, similar to
	/// [`with_custom_protocol`], but also sets the WebViewAssetLoader with the
	/// necessary domain (which is fixed as `<protocol>.assets`). This cannot be
	/// used in conjunction to `with_custom_protocol` for Android, as it
	/// changes the way in which requests are handled.
	#[cfg(feature = "protocol")]
	...
	/// Using a `http` scheme will allow mixed content when trying to fetch
	/// `http` endpoints and is therefore less secure but will match the
	/// behavior of the `<scheme>://localhost` protocols used on macOS and
	/// Linux.
	///
	/// The default value is `false`.
	fn with_https_scheme(self, enabled:bool) -> Self;
...
pub trait WebViewBuilderExtUnix<'a> {
	/// Consume the builder and create the webview inside a GTK container
	/// widget, such as GTK window.
	///
	/// - If the container is [`gtk::Box`], it is added using
	///   [`Box::pack_start(webview, true, true,
	///   0)`](gtk::prelude::BoxExt::pack_start).
	/// - If the container is [`gtk::Fixed`], its [size
	///   request](gtk::prelude::WidgetExt::set_size_request) will be set using
	///   the (width, height) bounds passed in and will be added to the
	///   container using [`Fixed::put`](gtk::prelude::FixedExt::put) using the
	///   (x, y) bounds passed in.
	/// - For all other containers, it will be added using
...
/// [`WebViewBuilder`] / [`WebView`] are the basic building blocks to construct
/// WebView contents and scripts for those who prefer to control fine grained
/// window creation and event handling. [`WebView`] presents the actual WebView
/// window and let you still able to perform actions on it.
pub struct WebView {
	webview:InnerWebView,
}

impl WebView {
	/// Create a [`WebView`] from from a type that implements
	/// [`HasWindowHandle`]. Note that calling this directly loses
	/// abilities to initialize scripts, add ipc handler, and many more before
	/// starting WebView. To benefit from above features, create a
	/// [`WebViewBuilder`] instead.
	///
	/// # Platform-specific:
	///
	/// - **Linux**: Only X11 is supported, if you want to support Wayland too,
	///   use [`WebViewExtUnix::new_gtk`].
	///
	///   Although this methods only needs an X11 window handle, you use
	/// webkit2gtk, so you still need to initialize gtk   by callling
	/// [`gtk::init`] and advance its loop alongside your event loop using
	/// [`gtk::main_iteration_do`].   Checkout the [Platform Considerations](https://docs.rs/wry/latest/wry/#platform-considerations) section in the crate root documentation.
	/// - **macOS / Windows**: The webview will auto-resize when the passed
	///   handle is resized.
	/// - **Linux (X11)**: Unlike macOS and Windows, the webview will not
	///   auto-resize and you'll need to call [`WebView::set_bounds`] manually.
	///
	...
	/// Create [`WebViewBuilder`] as a child window inside the provided
	/// [`HasWindowHandle`].
	///
	/// ## Platform-specific
	///
	...
	pub fn id(&self) -> WebViewId { self.webview.id() }

	/// Get the current url of the webview
	pub fn url(&self) -> Result<String> { self.webview.url() }
	...
	/// Evaluate and run javascript code with callback function. The evaluation
	/// result will be serialized into a JSON string and passed to the callback
	/// function.
	///
	/// Exception is ignored because of the limitation on windows. You can catch
	/// it yourself and return as string as a workaround.
	///
	...
	pub fn print(&self) -> Result<()> { self.webview.print() }
	...
	pub fn cookies(&self) -> Result<Vec<cookie::Cookie<'static>>> { self.webview.cookies() }
	...
	pub fn open_devtools(&self) { self.webview.open_devtools() }
	...
	#[cfg(any(debug_assertions, feature = "devtools"))]
	pub fn close_devtools(&self) { self.webview.close_devtools() }
	...
	/// - **Windows / Android / iOS:** Not supported.
	#[cfg(any(debug_assertions, feature = "devtools"))]
	pub fn is_devtools_open(&self) -> bool { self.webview.is_devtools_open() }
	...
	pub fn zoom(&self, scale_factor:f64) -> Result<()> { self.webview.zoom(scale_factor) }
	...
	pub fn load_url(&self, url:&str) -> Result<()> { self.webview.load_url(url) }
	...
	pub fn load_html(&self, html:&str) -> Result<()> { self.webview.load_html(html) }

	/// Clear all browsing data
	pub fn clear_all_browsing_data(&self) -> Result<()> { self.webview.clear_all_browsing_data() }

	pub fn bounds(&self) -> Result<Rect> { self.webview.bounds() }
	...
	pub fn set_bounds(&self, bounds:Rect) -> Result<()> { self.webview.set_bounds(bounds) }

	/// Shows or hides the webview.
	pub fn set_visible(&self, visible:bool) -> Result<()> { self.webview.set_visible(visible) }

	/// Try moving focus to the webview.
	pub fn focus(&self) -> Result<()> { self.webview.focus() }
	...
	pub fn focus_parent(&self) -> Result<()> { self.webview.focus_parent() }
}
...
pub enum DragDropEvent {
	...
	Enter {
		...
		paths:Vec<PathBuf>,
		/// Position of the drag operation, relative to the webview top-left
		/// corner.
		position:(i32, i32),
	},
...
pub fn webview_version() -> Result<String> { platform_webview_version() }

/// The [memory usage target level][1]. There are two levels 'Low' and 'Normal'
/// and the default level is 'Normal'. When the application is going inactive,
/// setting the level to 'Low' can significantly reduce the application's memory
/// consumption.
///
...
pub enum MemoryUsageLevel {
	/// The 'Normal' memory usage. Applications should set this level when they
	/// are becoming active.
	#[default]
	Normal,
	/// The 'Low' memory usage. Applications can reduce memory comsumption by
	/// setting this level when they are becoming inactive.
	Low,
}
...
pub trait WebViewExtWindows {
	...
	///
	/// Requires WebView2 Runtime version 101.0.1210.39 or higher, returns error
	/// on older versions, see https://learn.microsoft.com/en-us/microsoft-edge/webview2/release-notes/archive?tabs=dotnetcsharp#10121039
	fn set_theme(&self, theme:Theme) -> Result<()>;

	/// Sets the [memory usage target level][1].
	///
	/// When to best use this mode depends on the app in question. Most commonly
	/// it's called when the app's visiblity state changes.
	///
	/// Please read the [guide for WebView2][2] for more details.
	///
	/// This method uses a WebView2 API added in Runtime version 114.0.1823.32.
	/// When it is used in an older Runtime version, it does nothing.
	///
	/// [1]: https://learn.microsoft.com/en-us/dotnet/api/microsoft.web.webview2.core.corewebview2memoryusagetargetlevel
	...
	/// Attaches this webview to the given HWND and removes it from the current
	/// one.
...
impl WebViewExtWindows for WebView {
	fn controller(&self) -> ICoreWebView2Controller { self.webview.controller.clone() }

	fn set_theme(&self, theme:Theme) -> Result<()> { self.webview.set_theme(theme) }

	fn set_memory_usage_level(&self, level:MemoryUsageLevel) -> Result<()> {
		self.webview.set_memory_usage_level(level)
	}

	fn reparent(&self, hwnd:isize) -> Result<()> { self.webview.reparent(hwnd) }
}
...
pub trait WebViewExtUnix: Sized {
	...
	/// Attaches this webview to the given Widget and removes it from the
	/// current one.
...
impl WebViewExtUnix for WebView {
	...
	fn webview(&self) -> webkit2gtk::WebView { self.webview.webview.clone() }
...
pub trait WebViewExtMacOS {
	...
	/// Attaches this webview to the given NSWindow and removes it from the
	/// current one.
...
impl WebViewExtMacOS for WebView {
	fn webview(&self) -> Retained<WryWebView> { self.webview.webview.clone() }

	fn manager(&self) -> Retained<WKUserContentController> { self.webview.manager.clone() }

	fn ns_window(&self) -> Retained<NSWindow> { self.webview.webview.window().unwrap().clone() }

	fn reparent(&self, window:*mut NSWindow) -> Result<()> { self.webview.reparent(window) }
...
impl WebViewExtAndroid for WebView {
	fn handle(&self) -> JniHandle { JniHandle }
}
