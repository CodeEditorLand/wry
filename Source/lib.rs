...
//!
//! let webview = WebViewBuilder::new(&window).with_url("https://tauri.app").build().unwrap();
//! ```
//!
...
//!
//! let webview = builder.with_url("https://tauri.app").build().unwrap();
//! ```
//!
//! ## Child webviews
//!
...
pub struct WebViewAttributes {
	...
	/// A list of custom loading protocols with pairs of scheme uri string and
	/// a handling closure.
	///
	...
	/// to send requests with native `fetch` and `XmlHttpRequest` APIs. Here
	...
	/// using `window.ipc.postMessage("insert_message_here")` to host Rust
	/// code.
	...
	/// files on `<input type="file">` forms. Also note, that it's not
	/// possible to manually set the value of a `<input type="file">` via
	...
	/// the url being downloaded from and and the second is a mutable
	/// `PathBuf` reference that (possibly) represents where the file will be
	/// downloaded to. The latter parameter can be used to set the download
	/// location by assigning a new path to it, the assigned path _must_ be
	...
	/// successful or not. The closure takes a `String` representing the URL
	/// of the original download request, an `Option<PathBuf>` potentially
	...
	///
	/// Note this only enables devtools to the webview. To open it, you can
	/// call [`WebView::open_devtools`], or right click the page and open it
	...
	/// Indicates whether horizontal swipe gestures trigger backward and
...
impl<'a> WebViewBuilder<'a> {
	/// Create a [`WebViewBuilder`] from a type that implements
	/// [`HasWindowHandle`].
	///
	/// # Platform-specific:
	///
	...
	//// The closure takes two parameters, the first is a `String` representing
	//// the url being downloaded from and and the
...
impl Default for PlatformSpecificWebViewAttributes {
	fn default() -> Self { Self { data_store_identifier:None } }
}
...
impl Default for PlatformSpecificWebViewAttributes {
	fn default() -> Self {
		Self {
			...
			browser_accelerator_keys:true, /* This is WebView2's default
			                                * behavior */
			theme:None,
			use_https:false, /* To match macOS & Linux behavior in the
			                  * context of mixed content. */
...
impl WebView {
	...
	/// - Panics if the provided handle was not supported or invalid.
	/// - Panics on Linux, if [`gtk::init`] was not called in this thread.
	pub fn new(window:&impl HasWindowHandle) -> Result<Self> { WebViewBuilder::new(window).build() }
...
impl WebViewExtMacOS for WebView {
	fn webview(&self) -> cocoa::base::id { self.webview.webview }

	fn manager(&self) -> cocoa::base::id { self.webview.manager }
	...
	fn reparent(&self, window:cocoa::base::id) -> Result<()> { self.webview.reparent(window) }
