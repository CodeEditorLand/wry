...
use html5ever::{LocalName, interface::QualName, namespace_url, ns, tendril::TendrilSink};
use http::{
	Request,
...
mod main_pipe;
use main_pipe::{CreateWebViewAttributes, MAIN_PIPE, MainPipe, MainPipeState, WebViewMessage};
...
///
/// This function must be run on the thread where the [`JNIEnv`] is registered
/// and the looper is local, hence the requirement for a [`ThreadLooper`].
pub unsafe fn android_setup(
	...
	// we must create the WebChromeClient here because it calls
	// `registerForActivityResult`, which gives an `LifecycleOwners must call
	// register before they are STARTED.` error when called outside the onCreate
	// hook
...
impl InnerWebView {
	...
	pub fn new(
		...
		REQUEST_HANDLER.replace(Some(UnsafeRequestHandler::new(Box::new(
			move |webview_id:&str, mut request, is_document_start_script_enabled| {
				let uri = request.uri().to_string();
				if let Some((custom_protocol_uri, custom_protocol_closure)) = custom_protocols
					.iter()
                    ...
                    .unwrap_or_default();
								...
								if should_inject_scripts && !initialization_scripts.is_empty() {
									...
									with_html_head(&mut document, |head| {
										// iterate in reverse order since we are prepending each
										// script to the head tag
	...
	pub fn print(&self) -> crate::Result<()> { Ok(()) }
	...
	pub fn zoom(&self, _scale_factor:f64) -> Result<()> { Ok(()) }
	...
	pub fn cookies(&self) -> Result<Vec<cookie::Cookie<'static>>> { Ok(Vec::new()) }

	pub fn bounds(&self) -> Result<crate::Rect> { Ok(crate::Rect::default()) }
...
impl JniHandle {
	...
	/// Provided function will be provided with the jni evironment, Android
...
///
/// The closure takes the JNI env, the Android activity instance and the
/// possibly null webview.
