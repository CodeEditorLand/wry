...
macro_rules! android_binding {
	...
	// use imported `android_setup` just to force the import path to use
	// `wry::{}` as the macro breaks without braces
		...
		if let Some(response) = response {
			...
			let response_headers = {
				...
				for (name, value) in headers.iter() {
					// WebResourceResponse will automatically generate
					// Content-Type and Content-Length headers so we should
			...
			let web_resource_response_class =
