...
static CHANNEL:Lazy<(Sender<WebViewMessage>, Receiver<WebViewMessage>)> =
	Lazy::new(|| bounded(8));
		...
		if let Ok(message) = CHANNEL.1.recv() {
			match message {
				WebViewMessage::CreateWebView(attrs) => {
					...
					let string_class =
						self.env.find_class("java/lang/String")?;
					let initialization_scripts_array =
						self.env.new_object_array(
							initialization_scripts.len() as i32,
							string_class,
							self.env.new_string("")?,
						)?;
					for (i, script) in
						initialization_scripts.into_iter().enumerate()
					{
					...
					if let Some(on_webview_created) = on_webview_created {
						if let Err(e) = on_webview_created(super::Context {
							...
							#[cfg(feature = "tracing")]
							tracing::warn!(
								"failed to run webview created hook: {e}"
							);
						}
					}
...
pub(crate) struct CreateWebViewAttributes {
	...
	pub on_webview_created:
		Option<Box<dyn Fn(super::Context) -> JniResult<()> + Send>>,
	pub user_agent:Option<String>,
	pub initialization_scripts:Vec<String>,
}

// SAFETY: only use this when you are sure the span will be dropped on the same
// thread it was entered
#[cfg(feature = "tracing")]
