...
use windows::{
	...
	core::{HSTRING, Interface, PCWSTR, PWSTR, s, w},
};
...
impl InnerWebView {
	...
	fn create_controller(
		...
		// as it uses an mspc::channel under the hood, so we can avoid using two
		// channels by manually creating the callback handler and use
		// webview2_com::with_with_bump
	...
	fn init_webview(
		...
		// calls. We're taking it in the local variable and then just ignoring it
		// because all of the event handlers are registered for the life of the
		// webview, but if we wanted to be able to remove them later we would hold
		// onto them in self.
		...
		for (js, _) in attributes.initialization_scripts.iter().filter(|(_, for_main)| !*for_main) {
		...
		if let Some(mut url) = attributes.url {
			if let Some(pos) = url.find("://") {
				...
				if custom_protocols.contains(name) {
					// WebView2 supports non-standard protocols only on Windows 10+, so we have to
					// use this workaround See https://github.com/MicrosoftEdge/WebView2Feedback/issues/73
	...
	unsafe fn load_extensions(webview:&ICoreWebView2, extension_path:&PathBuf) -> Result<()> {
		let profile =
			webview.cast::<ICoreWebView2_13>()?.Profile()?.cast::<ICoreWebView2Profile7>()?;
...
/// Public APIs
impl InnerWebView {
	pub fn id(&self) -> crate::WebViewId { &self.id }
	...
	unsafe fn cookie_from_win32(cookie:ICoreWebView2Cookie) -> Result<cookie::Cookie<'static>> {
		...
		let expires = match expires {
			...
			datetime => {
				cookie::time::OffsetDateTime::from_unix_timestamp(datetime as _)
					.ok()
		...
		unsafe {
			webview.CookieManager()?.GetCookies(
				...
				&GetCookiesCompletedHandler::create(Box::new(move |error_code, cookies| {
					...
					tx.send(cookies).map_err(|_| windows::core::Error::from(E_UNEXPECTED))
				})),
			)?;
		}
