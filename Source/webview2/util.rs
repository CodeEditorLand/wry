...
pub fn dpi_to_scale_factor(dpi:u32) -> f64 { dpi as f64 / BASE_DPI as f64 }
...
pub unsafe fn hwnd_dpi(hwnd:HWND) -> u32 {
	if let Some(GetDpiForWindow) = *GET_DPI_FOR_WINDOW {
		...
		if IsProcessDPIAware().as_bool() {
			// If the process is DPI aware, then scaling must be handled by the
			// application using this DPI value.
			GetDeviceCaps(hdc, LOGPIXELSX) as u32
		} else {
			// If the process is DPI unaware, then scaling is performed by the
			// OS; we thus return 96 (scale factor 1.0) to prevent the window
			// from being re-scaled by both the application and the WM.
			BASE_DPI
		}
	}
}
