...
pub struct DragDropTarget {
	...
	enter_is_valid:UnsafeCell<bool>, /* If the currently hovered item is not valid there must
	                                  * not be any `HoveredFileCancelled` emitted */
}
		...
		match data_obj.as_ref().expect("Received null IDataObject").GetData(&drop_format) {
			Ok(medium) => {
				...
				for i in 0..item_count {
					...
					// character. Previously, this was using a fixed size array of MAX_PATH
					// length, but the Windows API allows longer paths under certain
					// circumstances.
