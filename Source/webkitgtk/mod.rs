...
use webkit2gtk::{
	...
	WebContextExt as Webkit2gtkWeContextExt,
	WebView,
	WebViewExt,
...
impl Drop for InnerWebView {
	fn drop(&mut self) { unsafe { self.webview.destroy() } }
}
...
impl InnerWebView {
	...
	fn cookie_from_soup_cookie(mut cookie:soup::Cookie) -> cookie::Cookie<'static> {
		...
		let expires = match expires {
			Some(datetime) => {
				cookie::time::OffsetDateTime::from_unix_timestamp(datetime.to_unix())
					.ok()
			...
			.map(|cookies_manager| {
				cookies_manager.cookies(url, None::<&Cancellable>, move |cookies| {
					let cookies = cookies.map(|cookies| {
						cookies.into_iter().map(Self::cookie_from_soup_cookie).collect()
					});
	...
	where
			...
			self.webview.parent().and_then(|p| p.dynamic_cast::<gtk::Container>().ok())
		{
			...
			let container_type = container.type_().name();
			if container_type == "GtkBox" {
				container.dynamic_cast_ref::<gtk::Box>().unwrap().pack_start(
					&self.webview,
					true,
					true,
					0,
