...
use webkit2gtk::{
	...
	URISchemeRequestExt,
	URISchemeResponse,
	URISchemeResponseExt,
	WebContext,
	WebContextExt as Webkit2gtkContextExt,
	WebView,
	WebViewExt,
};
...
impl WebContextExt for super::WebContext {
	...
	fn flush_queue_loader(&self) { self.os.webview_uri_loader.clone().flush() }
	...
	fn register_automation(&mut self, webview:WebView) {
		if let (true, Some(app_info)) = (self.os.automation, self.os.app_info.take()) {
			self.os.context.connect_automation_started(move |_, auto| {
				...
				// To support this in the future, we would need a way to specify
				// the default WindowBuilder to use to create the window it
				// will use, and possibly "default" webview attributes.
				// Difficulty comes in for controlling the owned Window that
...
/// Prevents an unknown concurrency bug with loading multiple URIs at the same
/// time on webkit2gtk.
///
/// Using the queue prevents data race issues with loading uris for multiple
/// [`WebView`]s in the same context at the same time. Occasionally, the one of
/// the [`WebView`]s will be clobbered and it's content will be injected into a
/// different [`WebView`].
///
...
/// ```
///
/// In that example, `webview-a` will load fine. `webview-b` will remain empty
/// as the uri was never loaded. `webview-c` will contain the content of both
/// `webview-b` and `webview-c` because it was triggered twice even through only
/// started once. The content injected will not be sequential, and often is
/// interjected in the middle of one of the other contents.
///
/// FIXME: We think this may be an underlying concurrency bug in webkit2gtk as
/// the usual ways of fixing threading issues are not working. Ideally, the
/// locks are not needed if we can understand the true cause of the bug.
...
impl WebViewUriLoader {
	...
	fn is_locked(&self) -> bool { self.lock.swap(true, SeqCst) }
