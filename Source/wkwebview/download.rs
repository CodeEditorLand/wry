use std::{ffi::c_void, path::PathBuf, ptr::null_mut, rc::Rc};
...
pub(crate) unsafe fn add_download_methods(decl:&mut ClassDecl) {
	...
	decl.add_method(
		...
		navigation_download_action as extern fn(&mut Object, Sel, id, id, id),
	);

	decl.add_method(
		sel!(webView:navigationResponse:didBecomeDownload:),
		navigation_download_response as extern fn(&mut Object, Sel, id, id, id),
	);
}

pub extern fn download_policy(
	this:&Object,
	_:Sel,
	download:id,
	_:id,
...
pub extern fn download_did_finish(this:&Object, _:Sel, download:id) {
	unsafe {
