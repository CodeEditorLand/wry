...
use crate::{RequestAsyncResponder, WryWebView, wkwebview::WEBVIEW_IDS};
		...
		match cls {
			Some(mut cls) => {
				...
				cls.add_method(
					...
					start_task as extern fn(_, _, _, _),
				);

				cls.add_method(
					objc2::sel!(webView:stopURLSchemeTask:),
					stop_task as extern fn(_, _, _, _),
				);
		...
		if !function.is_null() {
			...
			match http_request.body(sent_form_body) {
				Ok(final_request) => {
					let responder:Box<dyn FnOnce(HttpResponse<Cow<'static, [u8]>>)> =
						Box::new(move |sent_response| {
							...
							/// Task may not live longer than async custom
							/// protocol handler.
							///
							/// There are roughly 2 ways to cause segfault:
							/// 1. Task has stopped. pointer of the task not
							///    valid anymore.
							/// 2. Task had stopped, but the pointer of the task
							///    has allocated to a new task. Outdated custom
							///    handler may call to the new task instance and
							///    cause segfault.
							...
							unsafe fn response(
								// FIXME: though we give it a static lifetime, it's not guaranteed
								// to be valid.
								task:&'static ProtocolObject<dyn WKURLSchemeTask>,
								// FIXME: though we give it a static lifetime, it's not guaranteed
								// to be valid.
								...
								// default: application/octet-stream, but should be provided by the
								// client
                ...
                .unwrap();
								...
								// MIGRATE NOTE: we copied the content to the NSData because content
								// will be freed when out of scope but NSData will also free
								// the content when it's done and cause doube free.
			...
			#[cfg(feature = "tracing")]
			tracing::warn!(
				"Either WebView or WebContext instance is dropped! This handler shouldn't be \
				 called."
			);
		}
	}
