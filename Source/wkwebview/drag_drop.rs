...
extern fn dragging_updated(this:&mut Object, sel:Sel, drag_info:id) -> NSDragOperation {
	...
	if !listener(DragDropEvent::Over { position }) {
		...
		if os_operation == 0 {
			// 0 will be returned for a drop on any arbitrary location on the
			// webview. We'll override that with NSDragOperationCopy.
			NSDragOperationCopy
		} else {
			// A different NSDragOperation is returned when a file is hovered
			// over something like a <input type="file">, so we'll make sure
...
extern fn dragging_exited(this:&mut Object, sel:Sel, drag_info:id) {
	let listener = unsafe { get_handler(this) };
...
pub(crate) unsafe fn add_drag_drop_methods(decl:&mut ClassDecl) {
	...
	decl.add_method(
		sel!(draggingEntered:),
		dragging_entered as extern fn(&mut Object, Sel, id) -> NSDragOperation,
	);

	decl.add_method(
		sel!(draggingUpdated:),
		dragging_updated as extern fn(&mut Object, Sel, id) -> NSDragOperation,
	);
