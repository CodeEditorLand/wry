	...
	fn new_ns_view(
							...
							let responder:Box<dyn FnOnce(HttpResponse<Cow<'static, [u8]>>)> =
								Box::new(move |sent_response| {
									...
									unsafe fn response(
										...
										// default: application/octet-stream, but should be provided
										// by the client
		...
		unsafe {
			...
			let data_store:id = match (
				...
				(true, ..) => msg_send![class!(WKWebsiteDataStore), nonPersistentDataStore],
				// if data_store_identifier is given and custom data stores are available, use
				// custom store
			...
			let cls = match ClassDecl::new("WryWebView", class!(WKWebView)) {
				...
				Some(mut decl) => {
					#[cfg(target_os = "macos")]
					{
						...
						// FIXME: When the webview is a child webview, performKeyEquivalent always
						// return YES and stop propagating the event to the window, hence the
						// menu shortcut won't be triggered. However, overriding this method
						// also means the cmd+key event won't be handled in webview, which
						// means the key cannot be listened by JavaScript.
						if is_child {
							decl.add_method(
	...
	pub fn eval(&self, js:&str, callback:Option<impl Fn(String) + Send + 'static>) -> Result<()> {
		if let Some(scripts) = &mut *self.pending_scripts.lock().unwrap() {
			...
			unsafe {
				...
				// we need to check if the callback exists outside the handler otherwise it's a
				// segfault
...
impl NSString {
	fn new(s:&str) -> Self {
		...
		NSString(unsafe {
			...
			// The thing is allocated in rust, the thing must be set to autorelease in rust
			// to relinquish control or it can not be released correctly in OC runtime
