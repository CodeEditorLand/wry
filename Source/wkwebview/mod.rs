...
use crate::{Error, RGBA, Rect, RequestAsyncResponder, Result, WebViewAttributes, util::Counter};
#[cfg(feature = "mac-proxy")]
use crate::{
	proxy::ProxyConfig,
	wkwebview::proxy::{
...
impl InnerWebView {
	...
	fn new_ns_view(
		...
		unsafe {
			...
			// Incognito mode
			let (os_major_version, ..) = util::operating_system_version();
			#[cfg(target_os = "macos")]
			...
			let data_store = match (
				...
				(true, ..) => WKWebsiteDataStore::nonPersistentDataStore(),
			...
			let webview = {
				...
				let webview:Retained<WryWebView> = objc2::msg_send_id![super(webview), initWithFrame:frame configuration:&**config];
			...
			#[cfg(target_os = "ios")]
			{
				...
				// let scroll_view: Retained<UIScrollView> = webview.ivars().scrollView; //
				// FIXME: not test yet
...
r#"Object.defineProperty(window, 'ipc', {
  value: Object.freeze({postMessage: function(s) {window.webkit.messageHandlers.ipc.postMessage(s);}})
	...
	pub fn print_with_options(&self, _options:&PrintOptions) -> crate::Result<()> {
		...
		unsafe {
			let can_print =
				self.webview.respondsToSelector(objc2::sel!(printOperationWithPrintInfo:));
			if can_print {
	...
	unsafe fn cookie_from_wkwebview(cookie:&NSHTTPCookie) -> cookie::Cookie<'static> {
		...
		let same_site = match same_site {
			...
			Some(policy) if policy.as_ref() == NSHTTPCookieSameSiteStrict => {
		...
		let expires = match expires {
			Some(datetime) => {
				cookie::time::OffsetDateTime::from_unix_timestamp(
...
pub fn platform_webview_version() -> Result<String> {
	unsafe {
		let bundle =
			NSBundle::bundleWithIdentifier(&NSString::from_str("com.apple.WebKit")).unwrap();

		let dict = bundle.infoDictionary().unwrap();

		let webkit_version = dict.objectForKey(&NSString::from_str("CFBundleVersion")).unwrap();
