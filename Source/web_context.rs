...
///
/// A browser would have a context for all the normal tabs and a different
/// context for all the private/incognito tabs.
///
/// # Warning
/// If [`WebView`] is created by a WebContext. Dropping `WebContext` will cause
/// [`WebView`] lose some actions like custom protocol on Mac. Please keep both
/// instances when you still wish to interact with them.
///
...
impl WebContext {
	...
	pub fn data_directory(&self) -> Option<&Path> { self.data.data_directory() }
...
#[cfg(not(gtk))]
impl WebContextImpl {
	fn new(_data:&WebContextData) -> Self { Self }
